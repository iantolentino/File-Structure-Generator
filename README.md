...
def parse_file_structure(tree_string):
    lines = tree_string.split('\n')
    folder_stack = [{'name': 'root', 'path': '/', 'children': []}]
    all_items = []
    ...
    return {
        ...
        'operations': all_items
    }

"I have a file tree structure provided by a user. Parse it using this algorithm:

For each line, count the number of vertical bars (│) at the start — this is the depth level
Extract the name by removing all tree characters (├, ─, └, │, spaces)
Determine if it's a folder (ends with /) or file
Maintain a stack of current folders based on depth
Create files/folders in the correct parent directory
Output a list of creation operations in order (folders first, then files)
Input: [user's tree string] Output: List of { action, name, path, parentPath } objects"
